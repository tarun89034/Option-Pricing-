    ...
    try:
        ...
        if not days_str:
            return {
                "statusCode": 400,
                "headers": CORS_HEADERS,
                "body": json.dumps({"error": "Missing required parameter: days_to_expiry"}),
            }
        ...
        if option_type not in ("call", "put"):
            return {
                "statusCode": 400,
                "headers": CORS_HEADERS,
                "body": json.dumps({"error": "option_type must be 'call' or 'put'"}),
            }
        ...
        try:
            ...
            return {
                "statusCode": 400,
                "headers": CORS_HEADERS,
                "body": json.dumps({"error": "days_to_expiry must be a positive integer"}),
            }
        ...
        if strike_str is not None:
            try:
                ...
                return {
                    "statusCode": 400,
                    "headers": CORS_HEADERS,
                    "body": json.dumps({"error": "strike must be a positive number"}),
                }
        else:
            K = S  # ATM
        ...
        # Monte Carlo (with reduced simulations for speed in serverless)
        ...
        # Binomial Tree (reduced steps for serverless speed)
